...
def train_models_and_calc_scores_for_n_fold_cv(
    ...
    training_ids_per_fold, testing_ids_per_fold = make_train_and_test_row_ids_for_n_fold_cv(
        n_examples= N, n_folds= n_folds, random_state= random_state)

    # TODO loop over folds and compute the train and test error
    # for the provided estimator
    for f in range(n_folds):
        train_IDs = training_ids_per_fold[f]
        test_IDs = testing_ids_per_fold[f]
        ...
        train_error_per_fold[f] = calc_root_mean_squared_error(y_train, yhat_train)
        test_error_per_fold[f] = calc_root_mean_squared_error(y_test, yhat_test)
...
def make_train_and_test_row_ids_for_n_fold_cv(
    ...
    * The total size of train and test ids for any fold is equal to N
    ...
    #shuffle the order of ID's (make an array of length N containing 
    # numbers 0, 1, 2, ..., N, and use a random permutation/shuffle function)
    row_IDs = np.arange(n_examples, dtype= int)
    shuffled_IDs = random_state.permutation(row_IDs)

    # TODO loop over folds, establish which indices belong each fold
    # cut the array into n_folds sections, storing each section as its own array
    sliced_IDs = np.array_split(shuffled_IDs, n_folds)


    for f in range(n_folds):
        # TODO assign those indices to the fold's test set
        # update test_ID's to hold one of the folds (for all n_folds)
        test_IDs = sliced_IDs[f]
        # TODO assign remaining indices to the fold's train set
        # ensure all remaining ID's end up in train_id's per fold (for all n_folds)
        train_IDs = np.concatenate([sliced_IDs[x] for x in range(n_folds) if x != f])

        train_ids_per_fold.append(train_IDs.astype(int))
        test_ids_per_fold.append(test_IDs.astype(int))
